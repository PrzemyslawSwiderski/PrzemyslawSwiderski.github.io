...
Can also assist the customers in the real time.

Interesting that Traeger grill company uses those services to deliver premiere customer experience.
...
It took Moderna only two days to find proper covid vaccine model by using Aws LLM.
...
Luxmed is also using Aws GenAI AWS services.
Luxmed Insurance is using AWS Sagemaker for risk modelling and costs calculation.

Allianz is using GenAI to classify data.
...
After war in ukraine broke, they were forced to migrate the infrastructure to AWS.
...
* security scans on large project are slow
* needs more precise prompts, less forgiving than ChatGPT

Due to law restrictions SLA includes that the code must stay internally in the bank AWS account.

The generic code model is extended with the one from PrivatBank to provide proper autocompletion.
...
GenAI is rule-changing in terms of time saved for the initial development.
Someone can create solution in much less time than before

The Biggest investor’s question, what will the GenAI startup do in 6 years?
What is the plan if every week braking changes are done in AI field.
...
It is hard for the smaller startups to compete with tech giants since they have much more resources and training data.

You don’t need ChatGPT if the problem can be solved done with 4 ifs in the code instead.

Your AI is as good as your data.

Some startups are not aware of legal actions needed to be taken when learning the model with customers data.
They are risking costly lawsuits.

Current AI market state resembles the dot-com bubble.

All agreed that there are too high expectations (fueled by hype) about the AI now.

## 3:00 PM - Platform engineering with Amazon EKS

Presentation done by the senior solutions architect covering best practices and tips when dealing with AWS
infrastructure.
...
[Karpenter](https://karpenter.sh/) to reduce and manage the cluster costs.
...
Always add version to prompt (e.g. Spring Boot) to help finding the solution.

Generated code can be not secure. Trust, but always verify.
...
When creating the solutions with GenAI we should be aware
of [Hallucinations](https://en.wikipedia.org/wiki/Hallucination_(artificial_intelligence)).
Which can result in responses that are authoritative but wrong.

They created a Chatbot which scans AWS services and finds potential loopholes.
